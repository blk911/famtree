...
function VmbStartLayoutBranchInner({ children }: { children: ReactNode }) {
  ...
  const [useSalonShell, setUseSalonShell] = useState<boolean | null>(null);

  useEffect(() => {
    if (!refreshMode) {
      setUseSalonShell(false);
      return;
    }
...
/** /vmb/start uses minimal shell for first ingest; refresh with workspace uses salon rail. */
