...
const HERO_COPY: Array<{ match: (path: string) => boolean; title: string; subtitle: string; hidden?: true }> = [
  ...
  { match: (path) => path === "/discovery" || path.startsWith("/discovery/"), title: "", subtitle: "", hidden: true },
  { match: (path) => path === "/msg-vault" || path.startsWith("/msg-vault/"), title: "Msg Vault", subtitle: "Private chats, threads, and notices" },
  { match: (path) => path === "/admin/discovery" || path.startsWith("/admin/discovery/"), title: "Discovery", subtitle: "Salon intelligence workspace", hidden: true },
  { match: (path) => path === "/admin/studios" || path.startsWith("/admin/studios/"), title: "AIH Studios", subtitle: "Legacy studio tooling", hidden: true },
