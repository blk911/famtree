...
/** Collapsible live preview — collapsed on load; ships with repository panel. */
export function MemberVideoCurrentPreview() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setEnabled(
          items.find((i) => i.isEnabled && i.videoUrl?.trim()) ??
            items.find((i) => i.isEnabled) ??
            null,
        );
      }
    } catch {
  ...
  const previewSrc = enabled?.videoUrl?.trim() || (enabled ? BUNDLED_INTRO_PATH : null);
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <div style={{ padding: "18px 20px 22px" }}>
          {loading && <p style={{ fontSize: 13, color: "#a8a29e", margin: 0 }}>Loading preview…</p>}
          {!loading && !enabled && (
            <p style={{ fontSize: 13, color: "#57534e", margin: 0, lineHeight: 1.5 }}>
              No repository row is enabled. Enable one in the panel above.
            </p>
          )}
          {!loading && enabled && (
            <>
              ...
              {previewSrc ? (
                ...
                <p style={{ fontSize: 13, color: "#b45309", margin: 0 }}>
                  Enabled row has no video URL — add an MP4 path in the repository above.
                </p>
              )}
            </>
          )}
...
/** Repository + live preview (single client chunk for /admin/tools). */
export function MemberVideoToolsPanels() {
  return (
