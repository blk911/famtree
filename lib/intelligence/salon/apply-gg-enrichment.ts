...
/**
 * Returns true when a URL matches a pattern that was generated by the IG resolver's
 * `generateCandidateUrls` function for appointment platforms.
 *
 * Generated appointment URLs must NOT be fed to the GG discovery engine as
 * "known good" URLs — they are hypothesis probes, not real evidence.
 *
 * Pattern: appointment-platform URL where the path ends with the handle
 * (or a simple variant: dotless, underscoreless).
 */
function isGeneratedAppointmentUrl(url: string, handle: string): boolean {
  ...
  // StyleSeat: styleseat.com/m/{handle}
  if (lower.includes("styleseat.com/m/")) {
    return [h, ul, dl, hl].some((v) => lower.endsWith(`/m/${v}`) || lower.endsWith(`/m/${v}/`));
  }
  // GlossGenius: {handle}.glossgenius.com
  if (lower.includes("glossgenius.com")) {
    return [h, ul, dl, hl].some((v) => lower.startsWith(`https://${v}.`) || lower.startsWith(`http://${v}.`));
  }
  // Vagaro: vagaro.com/{handle}
  if (lower.includes("vagaro.com/")) {
    return [h, ul, dl, hl].some((v) => lower.includes(`vagaro.com/${v}`) || lower.includes(`vagaro.com/us/${v}`));
  }
  // Booksy: booksy.com/en-us/{handle}
  if (lower.includes("booksy.com/")) {
    return [h, ul, dl, hl].some((v) => lower.endsWith(`/${v}`) || lower.endsWith(`/${v}/`));
  }
  // Square: {handle}.square.site
  if (lower.includes("square.site")) {
    return [h, ul, dl, hl].some((v) => lower.startsWith(`https://${v}.`) || lower.startsWith(`http://${v}.`));
  }
  return false;
}
...
export function upsertInputToGgEnrichInput(upsert: UpsertInput): GgEnrichmentInput {
  ...
  // Build the set of rejected probe URLs (from stored rejectedCandidateUrls + pattern detection).
  // Generated appointment-platform URLs must NOT reach the GG discovery engine as evidence.
  const rejectedUrls = new Set(
    (upsert.rejectedCandidateUrls ?? []).map((r) =>
      typeof r === "object" ? r.url : String(r),
    ).filter(Boolean),
  );

  const cleanLinkTrail = (upsert.linkTrailUrlsScanned ?? []).filter(
    (u) => !rejectedUrls.has(u) && !isGeneratedAppointmentUrl(u, handle),
  );
  ...
  return {
    ...
    linkTrailUrls: cleanLinkTrail,
    linkTrailUrlsScanned: cleanLinkTrail,
