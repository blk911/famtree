...
export async function sendApprovedInvitation(input: {
  ...
  const publicSnapshot: SentInvitePublicSnapshot = {
    ...
    valueLabel: approval.snapshot.valueLabel,
    totalValue: approval.snapshot.totalValue,
    savingsAmount: approval.snapshot.savingsAmount,
    offerPrice: approval.snapshot.offerPrice,
